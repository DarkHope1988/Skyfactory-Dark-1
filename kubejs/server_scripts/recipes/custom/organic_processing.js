// Skyfactory Dark - Organic Processing (Stage-0 Bio Loop)

ServerEvents.recipes(event => {
  const hasItem = id => {
    try {
      return !Item.of(id).isEmpty();
    } catch (e) {
      return false;
    }
  };
  // Leaf threads -> leaf bundles.
  event.shaped('sfd_comets:leaf_bundle', [
    'LL',
    'LL'
  ], {
    L: 'sfd_comets:leaf_threads'
  });

  // Leaf bundle + wood shavings -> first earth block.
  event.shapeless('sfd_comets:earth_block', [
    'sfd_comets:leaf_bundle',
    'sfd_comets:wood_shavings'
  ]);

  // Earth clumps can be compressed back into earth blocks.
  event.shaped('sfd_comets:earth_block', [
    'CC',
    'CC'
  ], {
    C: 'sfd_comets:earth_clump'
  });

  // Tree bark -> placeable bark block.
  event.shaped('sfd_comets:bark_block', [
    'BB',
    'BB'
  ], {
    B: 'sfd_comets:tree_bark'
  });

  // Dried worm + wood shavings => organic rod (OX / XO).
  event.shaped('sfd_comets:organic_rod', [
    'DW',
    'WD'
  ], {
    D: 'sfd_comets:dried_worm',
    W: 'sfd_comets:wood_shavings'
  });

  // Old items remain craftable for compatibility with existing content,
  // and become the Stage-1 bridge into stone progression.
  event.shapeless('sfd_comets:organic_dust', [
    'sfd_comets:leaf_threads',
    'sfd_comets:leaf_threads',
    'sfd_comets:leaf_threads',
    'sfd_comets:leaf_threads'
  ]);

  event.shapeless('sfd_comets:resin_fragment', [
    'sfd_comets:wood_shavings',
    'sfd_comets:leaf_threads'
  ]);

  // Stage 1 unlock chain (after first workbench).
  event.shaped('sfd_comets:compost_pile', [
    'DDD',
    'D D',
    'DDD'
  ], {
    D: 'sfd_comets:organic_dust'
  });

  event.shaped('sfd_comets:compost_pulp', [
    ' D ',
    'DRD',
    ' D '
  ], {
    D: 'sfd_comets:organic_dust',
    R: 'sfd_comets:resin_fragment'
  });

  event.shaped('minecraft:dirt', [
    'CC',
    'CC'
  ], {
    C: 'sfd_comets:compost_pile'
  });

  event.shaped('4x sfd_comets:packed_soil', [
    'DCD',
    'CDC',
    'DCD'
  ], {
    D: 'minecraft:dirt',
    C: 'sfd_comets:compost_pulp'
  });

  // Stage-1 fuel path: bark processed into compact furnace fuel.
  event.shaped('4x sfd_comets:bark_briquette', [
    ' B ',
    'BRB',
    ' B '
  ], {
    B: 'sfd_comets:tree_bark',
    R: 'sfd_comets:resin_fragment'
  }).id('skyfactorydark:stage1/bark_briquette');

  // Alternative fuer Plattformbau:
  // 4x Earth -> 4x Bau-Erdblock (kein Materialgewinn, aber klare Trennung).
  event.shaped('4x sfd_comets:builder_earth_block', [
    'EE',
    'EE'
  ], {
    E: 'sfd_comets:earth_block'
  });

  // Stage-0 Arbeitspodest (eigener Block), bewusst als 3x3-Rezept gebalanced.
  const podestId = Platform.isLoaded('sfd_comets') ? 'sfd_comets:bio_podest' : 'sfd_comets:bio_podest';
  event.shaped(podestId, [
    'BRB',
    'EOE',
    'BBB'
  ], {
    B: 'sfd_comets:builder_earth_block',
    R: 'sfd_comets:resin_fragment',
    E: 'sfd_comets:earth_block',
    O: 'sfd_comets:organic_rod'
  }).id('skyfactorydark:stage0/bio_podest_trapdoor');

  // Stage-0 Bio Backpack (custom addon mod item, exactly 9 slots).
  if (Platform.isLoaded('sfd_comets')) {
    event.shaped('sfd_comets:bio_backpack', [
      'RR',
      'WP'
    ], {
      R: 'sfd_comets:organic_rod',
      W: 'sfd_comets:dried_worm',
      P: 'sfd_comets:packed_soil'
    }).id('skyfactorydark:stage0/bio_beutel_9slot');
  }

  // Stage-0 Growth Paste: custom right-click growth item (bonemeal behavior).
  event.shaped('2x sfd_comets:bio_growth_paste', [
    'LR',
    'DW'
  ], {
    L: 'sfd_comets:leaf_threads',
    R: 'sfd_comets:resin_fragment',
    D: 'sfd_comets:organic_dust',
    W: 'sfd_comets:dried_worm'
  }).id('skyfactorydark:stage0/bio_growth_paste');

  // Tier-2 growth paste: 8 uses (durability-based).
  // Guarded so recipe load does not hard-fail if the target item isn't present yet.
  if (hasItem('sfd_comets:bio_growth_paste_t2')) {
    event.shaped('sfd_comets:bio_growth_paste_t2', [
      'PP',
      'PP'
    ], {
      P: 'sfd_comets:bio_growth_paste'
    }).id('skyfactorydark:stage1/bio_growth_paste_t2');
  }

  event.shaped('sfd_comets:crude_mallet', [
    ' RR',
    ' SR',
    'S  '
  ], {
    S: 'minecraft:stick',
    R: 'sfd_comets:resin_fragment'
  });

  event.shaped('minecraft:string', [
    'FF',
    'FF'
  ], {
    F: 'sfd_comets:organic_fiber'
  });

  event.shaped('minecraft:dirt', [
    'RR',
    'RR'
  ], {
    R: 'sfd_comets:raw_soil_chunk'
  });

  // Resin has direct early utility.
  event.shapeless(Item.of('minecraft:stick', 2), [
    'sfd_comets:resin_fragment'
  ]);

  // Optional resin path for treated wood conversion.
  event.shapeless('sfd_comets:treated_hollow_bark_block', [
    'sfd_comets:hollow_bark_block',
    'sfd_comets:resin_fragment'
  ]);

  // Workbench unlock: still 2x2 craftable, but gated behind first plank loop.
  event.shaped('minecraft:crafting_table', [
    'PO',
    'PP'
  ], {
    P: 'minecraft:oak_planks',
    O: 'sfd_comets:organic_rod'
  }).id('skyfactorydark:stage0/workbench_unlock');
});


